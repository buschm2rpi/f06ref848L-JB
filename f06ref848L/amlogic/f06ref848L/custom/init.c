...
static void import_kernel_nv(char *name, int in_qemu)
{
    ...
    *value++ = 0;
    ...
    if (!in_qemu)
    {
        ...
        } else if (!strcmp(name,"androidboot.resolution")) {
...
static int logo_init_action(int nargs, char **args)
{
    char tmp[PROP_VALUE_MAX];

    INFO("logo_init\n");
    if (resolution[0]) {
        INFO("symlink " INIT_IMAGE_TEMP " %s\n", resolution, INIT_IMAGE_FILE);
        snprintf(tmp, sizeof(tmp), INIT_IMAGE_TEMP, resolution);
        if (symlink(tmp, INIT_IMAGE_FILE)) {
            ERROR("symlink error (%s)", strerror(errno));
        }
    }
    return 0;
}
