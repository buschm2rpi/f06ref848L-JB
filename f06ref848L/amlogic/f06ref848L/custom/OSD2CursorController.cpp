...
static void convertRGBAtoBGRA(const SkBitmap& bm) {
    ...
    while (p < stop - 4) {
        // swap red/blue
        //LOGV("%02x %02x %02x %02x", p[0], p[1], p[2], p[3]);
        uint8_t t = p[0];
        p[0] = p[2];
        p[2] = t;
        p += 4;
...
void OSD2CursorController::drawCursor(uint8_t *buffer, ssize_t buflen, uint32_t line_len) {
    ...
    if (mTvOutEnabled)
        rotation = 0;
    LOGV("rotation=%d mHWrotation=%d mRotation=%d", rotation, mHWrotation, mRotation);
    LOGV("line_len=%d", line_len);

    if (!mIcon.isValid()) {
        LOGD("mIcon not valid");
        return;
    }
    ...
    if (rotation == 90 || rotation == 270)
        SkTSwap(osdw, osdh);
    LOGV("mIcon.bitmap=%dx%d scale_shift=%dx%x osd=%dx%d",
...
void OSD2CursorController::setCursor() {
    if (mFb1fd < 0) {
        ...
        LOGV("setCursor, open, mFb1fd=%d", mFb1fd);
    }
    if (mFb1fd >= 0) {
...
#else
        if (!mIcon.isValid()) {
            LOGV("setCursor, mIcon not valid");
            return;
        }
        ...
        if (!ioctl(mFb1fd, FBIOGET_VSCREENINFO, &vinfo) &&
            ...
            /* get scale value */
            int scale;
            LOGV("vinfo. %d %d", vinfo.xres, vinfo.yres);

            xscale = 0;
            scale = vinfo.xres / cursorw;
            LOGV("cursorw=%d scale=%d", cursorw, scale);
            while (scale > 1) {
                xscale++;
                scale >>= 1;
            }
            ...
            LOGV("cursorh=%d scale=%d", cursorh, scale);
            while (scale > 1) {
                yscale++;
                scale >>= 1;
            }
            /* resize osd2 */
            vinfo.xres = vinfo.xres_virtual = 22; //cursorw << xscale;
            vinfo.yres = vinfo.yres_virtual = 28; //cursorh << yscale;
...
OSD2CursorController::OSD2CursorController() :
    ...
    if (mHWrotation != 0 && mHWrotation != 90 &&
        mHWrotation != 180 && mHWrotation != 270)
        mHWrotation = 0;
...
void OSD2CursorController::setIcon(const SpriteIcon& icon) {
    LOGV("setIcon line=%d, mFb1fd=%d", __LINE__, mFb1fd);
    if (icon.isValid()) {
...
void OSD2CursorController::setPosition(short x, short y) {
    ...
    //    return;
    if (mFb1fd < 0) {
        LOGV("setPosition line=%d, mFb1fd=%d", __LINE__, mFb1fd);
        setCursor();
    }
    if (mFb1fd >= 0) {
...
#else
        if (!mIcon.isValid()) {
            LOGV("setPosition, mIcon not valid");
            return;
        }
        osdw = mIcon.bitmap.width() << xscale;
        osdh = mIcon.bitmap.height() << yscale;
        ...
        if ((mHWrotation == 0 && mRotation == ROTATION_0) ||
            ...
            }if((mHWrotation == 270 && mRotation == ROTATION_270)){
            ...
            return;
        if (DEBUG > 1) LOGV("setPosition %d, %d -> %d, %d disp=%d, %d  userrotation=%d, rotation=%d\n",
