...
public class PhoneWindowManager implements WindowManagerPolicy {
	...
	public void showNotification(){
        final String filename = "/sys/class/update_status_dev/update_status";
        String tickertext ="";
		String title ="touch update";
        if (new File(filename).exists()) {
            try {
				int n = Integer.parseInt(readLine(filename));
				Log.v(TAG, "showNotification()----------n="+n);
                if(n==0){
                    return ;
                }
                else if(n==1){
                    tickertext = "touch update succeed";
                }else if(n ==2){
                    tickertext = "touch update fail";
                }
                Notification notification=new Notification(com.android.internal.R.drawable.stat_sys_warning,title,System.currentTimeMillis());
                notification.flags = Notification.FLAG_AUTO_CANCEL;        
notification.setLatestEventInfo(mContext, title, tickertext, null);        
                nm.notify(0, notification);
            } catch (IOException ex) {
                Slog.w(TAG, "Couldn't read touch update state from " + filename + ": " + ex);
