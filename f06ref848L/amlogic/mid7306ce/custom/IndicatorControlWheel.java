...
public class IndicatorControlWheel extends IndicatorControl implements
	...
	private static final int MIN_ZOOM_CONTROL_DEGREES = 96;
    ...
    private int getTouchIndicatorIndex(double delta) {
		...
		double startRadians,endRadians;
        if (mCurrentLevel == 0) {
            endIndex = mSecondLevelStartIndex - 1;
			endRadians = mChildRadians[endIndex] + halfTouchSectorRadians;

			if( (mCameraPicker == null) && (mZoomControl != null)) {
				//if we didn't have camera picker icon, use the zoom icon
				startRadians = Math.toRadians(MIN_ZOOM_CONTROL_DEGREES) - halfTouchSectorRadians;
			} else {
        ...
        if ((delta >= startRadians) && (delta <= endRadians)) {
			//Log.d(TAG,"Check which indicator is touched.");
			...
			//this check method thought we should have more than 2 icons except zoom control
		...
		Log.d(TAG,"Check which indicator is touched failed");
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        double dx = event.getX() - mCenterX +35;
        double dy = mCenterY - event.getY()+10;
    ...
    private void presetFirstLevelChildRadians() {
		...
		else//the zoom control is not dynamic added , it always exists in this viewgroup
			mChildRadians[startIndex++] = 0;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (selectedIndex >= 1) {
            ...
            float innerR = (float) mShutterButtonRadius-18;
            float outerR = (float) (mShutterButtonRadius + mStrokeWidth +
                    EDGE_STROKE_WIDTH * 0.5) - 25;
